...
//			return;
	...
	anims[0] =	xr_new<type_motion0>()->setup( k, ini, section, "kill_enertion"			);	//1.	Èíåðöèîííîå äâèæåíèå âïåðåä îò ïîïàäàíèÿ â ãîëîâó 
	anims[1] =	xr_new<type_motion1>()->setup( k, ini, section, "kill_burst"			);	//2.	Èçðåøåòèòü ïóëÿìè
	anims[2] =	xr_new<type_motion2>()->setup( k, ini, section, "kill_shortgun"			);	//3.	Øîòãàí 

	anims[6] =	xr_new<type_motion3>()->setup( k, ini, section, "kill_headshot"			);	//4.	Õåäøîò (ïî âåðîÿòíîñòè), êðîìå 5 (4) 
	anims[4] =	xr_new<type_motion4>()->setup( k, ini, section, "kill_sniper_headshot"	);	//5.	Ñíàéïåðêà â ãîëîâó. 
	anims[5] =	xr_new<type_motion5>()->setup( k, ini, section, "kill_sniper_body"		);	//6.	Ñíàéïåðêà â òåëî. 
	anims[3] =	xr_new<type_motion6>()->setup( k, ini, section, "kill_grenade"			);	//7.	Ãðàíòà 

	anims[7] =	xr_new<type_motion7>()->setup( k, ini, section, "kill_electric"			);	//4.	Õåäøîò (ïî âåðîÿòíîñòè), êðîìå 5 (4) 
